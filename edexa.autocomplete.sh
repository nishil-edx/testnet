#!/usr/bin/env bash
#
# edeXa Bash Completion
# =======================
#
# Bash completion support for the `edeXa` command,
...
#
# 2. Open a new bash console, and type `edeXa [TAB][TAB]`
#
...
# 'edeXa (..)'. By reading entered command line parameters,
...
# _complete_edeXa finds which commands and subcommands have been specified
# on the command line and delegates to the appropriate function
# to generate possible options and subcommands for the last specified subcommand.
function _complete_edeXa() {
  ...
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} blocks" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} public-key" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} password" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} retesteth" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} rlp" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} validate-config" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} storage" ];    then _picocli_edeXa; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} blocks import" ];    then _picocli_edeXa_blocks; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} blocks export" ];    then _picocli_edeXa_blocks; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} public-key export" ];    then _picocli_edeXa_publickey; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} public-key export-address" ];    then _picocli_edeXa_publickey; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} password hash" ];    then _picocli_edeXa_password; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} rlp encode" ];    then _picocli_edeXa_rlp; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator generate-blockchain-config" ];    then _picocli_edeXa_operator; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator generate-log-bloom-cache" ];    then _picocli_edeXa_operator; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator x-backup-state" ];    then _picocli_edeXa_operator; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator x-restore-state" ];    then _picocli_edeXa_operator; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} storage revert-variables" ];    then _picocli_edeXa_storage; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} storage rocksdb" ];    then _picocli_edeXa_storage; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} storage rocksdb usage" ];    then _picocli_edeXa_storage_rocksdb; return $?; fi
  ...
  local cmds19=(storage rocksdb)
  local cmds20=(storage rocksdb usage)

  if CompWordsContainsArray "${cmds20[@]}"; then _picocli_edeXa_storage_rocksdb_usage; return $?; fi
  if CompWordsContainsArray "${cmds19[@]}"; then _picocli_edeXa_storage_rocksdb; return $?; fi
  if CompWordsContainsArray "${cmds18[@]}"; then _picocli_edeXa_storage_revertvariables; return $?; fi
  if CompWordsContainsArray "${cmds17[@]}"; then _picocli_edeXa_operator_xrestorestate; return $?; fi
  if CompWordsContainsArray "${cmds16[@]}"; then _picocli_edeXa_operator_xbackupstate; return $?; fi
  if CompWordsContainsArray "${cmds15[@]}"; then _picocli_edeXa_operator_generatelogbloomcache; return $?; fi
  if CompWordsContainsArray "${cmds14[@]}"; then _picocli_edeXa_operator_generateblockchainconfig; return $?; fi
  if CompWordsContainsArray "${cmds13[@]}"; then _picocli_edeXa_rlp_encode; return $?; fi
  if CompWordsContainsArray "${cmds12[@]}"; then _picocli_edeXa_password_hash; return $?; fi
  if CompWordsContainsArray "${cmds11[@]}"; then _picocli_edeXa_publickey_exportaddress; return $?; fi
  if CompWordsContainsArray "${cmds10[@]}"; then _picocli_edeXa_publickey_export; return $?; fi
  if CompWordsContainsArray "${cmds9[@]}"; then _picocli_edeXa_blocks_export; return $?; fi
  if CompWordsContainsArray "${cmds8[@]}"; then _picocli_edeXa_blocks_import; return $?; fi
  if CompWordsContainsArray "${cmds7[@]}"; then _picocli_edeXa_storage; return $?; fi
  if CompWordsContainsArray "${cmds6[@]}"; then _picocli_edeXa_validateconfig; return $?; fi
  if CompWordsContainsArray "${cmds5[@]}"; then _picocli_edeXa_operator; return $?; fi
  if CompWordsContainsArray "${cmds4[@]}"; then _picocli_edeXa_rlp; return $?; fi
  if CompWordsContainsArray "${cmds3[@]}"; then _picocli_edeXa_retesteth; return $?; fi
  if CompWordsContainsArray "${cmds2[@]}"; then _picocli_edeXa_password; return $?; fi
  if CompWordsContainsArray "${cmds1[@]}"; then _picocli_edeXa_publickey; return $?; fi
  if CompWordsContainsArray "${cmds0[@]}"; then _picocli_edeXa_blocks; return $?; fi

  # No subcommands were specified; generate completions for the top-level command.
  _picocli_edeXa; return $?;
}

# Generates completions for the options and subcommands of the `edeXa` command.
function _picocli_edeXa() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_edeXa_blocks() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_edeXa_publickey() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_edeXa_password() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_edeXa_retesteth() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_operator() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_edeXa_validateconfig() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_storage() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}

  local commands="revert-variables rocksdb"
  local flag_opts="-h --help -V --version"
  local arg_opts=""
...
function _picocli_edeXa_blocks_import() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_blocks_export() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
# Generates completions for the options and subcommands of the `export` subcommand.
function _picocli_edeXa_publickey_export() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_publickey_exportaddress() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_password_hash() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_rlp_encode() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_operator_generateblockchainconfig() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_operator_generatelogbloomcache() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_operator_xbackupstate() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_operator_xrestorestate() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}
...
function _picocli_edeXa_storage_revertvariables() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_edeXa_storage_rocksdb() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_edeXa_storage_rocksdb_usage() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
# `edeXa`, `edeXa.sh`, and `edeXa.bash` commands.
# Uses the bash `complete` builtin (see [6]) to specify that shell function
# `_complete_edeXa` is responsible for generating possible completions for the
# current word on the command line.
# The `-o default` option means that if the function generated no matches, the
# default Bash completions and the Readline default filename completions are performed.
complete -F _complete_edeXa -o default edeXa edeXa.sh edeXa.bash
